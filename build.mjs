...
## Private events
Bookable for private events: ${site.privatePackages.map((p) => `${L(p.name, 'en')} (${L(p.capacity, 'en')}) for ${L(p.best, 'en').toLowerCase()}`).join('; ')}. Inquiries: ${site.phone} or ${site.origin}${ROUTES.private.en}
